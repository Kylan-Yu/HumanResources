...
import { getLeaveMyPage, getLeaveProgress } from '@/api/leaveWorkflow'
...
const LeaveMyPage: React.FC = () => {
  ...
  const loadData = async (extra?: any) => {
    ...
    setLoading(true)
    try {
      const res = await getLeaveMyPage(params)
  ...
  const openDetail = async (id: number) => {
    const res = await getLeaveProgress(id)
    setDetail(res.data)
    setDetailOpen(true)
  }
  ...
  return (
    <Card title="我的申请">
      ...
      >
        <Form.Item name="applyNo" label="申请单号"><Input allowClear /></Form.Item>
      ...
      <Table
        ...
        columns={[
          ...
          {
            title: '操作',
            render: (_: any, row: any) => <Button type="link" onClick={() => openDetail(row.id)}>查看进度</Button>
          }
        ]}
        pagination={{
      ...
      <Drawer title="审批进度" width={760} open={detailOpen} onClose={() => setDetailOpen(false)}>
        ...
        <p><b>状态：</b>{detail?.application?.status}</p>
