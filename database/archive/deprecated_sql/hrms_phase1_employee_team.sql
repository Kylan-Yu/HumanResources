...
SELECT '琛ュ崱瀹℃壒娴佺▼V1', 'PATCH', 'ENABLED', 1, '榛樿琛ュ崱娴佺▼妯℃澘', NOW(), NOW(), 0
WHERE NOT EXISTS (
    SELECT 1
...
SELECT x.id, 1, '鐩村睘涓荤瀹℃壒', 'DIRECT_LEADER', NULL, NULL, NULL, 1, NOW(), NOW(), 0
FROM (
    SELECT id
    FROM `hr_workflow_template`
    WHERE `business_type` = 'PATCH'
...
INSERT INTO `hr_workflow_template` (`template_name`, `business_type`, `status`, `version_no`, `remark`, `created_time`, `updated_time`, `deleted`)
SELECT '鍔犵彮瀹℃壒娴佺▼V1', 'OVERTIME', 'ENABLED', 1, '榛樿鍔犵彮娴佺▼妯℃澘', NOW(), NOW(), 0
WHERE NOT EXISTS (
    SELECT 1
    FROM `hr_workflow_template`
